...
import { ChevronDown, ChevronUp, Paperclip, FileText, Sparkles, Server, Download, MessageSquareQuote } from 'lucide-react'
...
type MentionType = 'file' | 'skill' | 'mcp' | 'chat'
...
const MENTION_STYLES: Record<MentionType, { icon: typeof FileText; className: string }> = {
  ...
  chat: { icon: MessageSquareQuote, className: 'bg-[hsl(203_90%_55%/0.15)] text-[hsl(203_75%_45%)]' },
}
...
// ----- remarkMentions：将 @file: /skill: $chat: #mcp: 转为 mention:// link 节点 -----
...
export function remarkMentions() {
  return (tree: MdastParent) => {
    walkMdastText(tree, (node, index, parent) => {
      ...
      const mentionPattern = /@file:(\S+)|\/skill:(\S+)|\$chat:(\S+)|\/chat:(\S+)|#mcp:(\S+)/g
      ...
      while ((m = mentionPattern.exec(text)) !== null) {
        ...
        const mType: MentionType = m[1] ? 'file' : m[2] ? 'skill' : (m[3] || m[4]) ? 'chat' : 'mcp'
...
/** mention:// URL 匹配 */
const MENTION_URL_RE = /^mention:\/\/(file|skill|mcp|chat)\/(.+)$/
